...
.PARAMETER Credential
    ...
    Admin role could be one of the following:
        - Global Admin
        - Application Admin
...
Param(
    ...
    [Parameter(Mandatory=$false, HelpMessage="Logon credential for admin account. Will be prompted if not specified.")]
...
try
{
    ...
    if ($Credential -eq $null) {
        # let MSOnline prompt a login UI, which may allow MFA.
        Connect-MsolService -ErrorAction Stop -AzureEnvironment $AzureEnvironment
    }
    else{
